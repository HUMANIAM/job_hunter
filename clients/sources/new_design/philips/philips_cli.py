...
if __package__ in {None, ""}:
    raise SystemExit(
        ...
        "python -m clients.sources.new_design.philips.philips_cli ..."
    )
...
DEFAULT_URLS_PATH = Path("data/refactor/jobs/philips/urls.md")
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Collect Philips vacancy links using the new source design."
    )
    parser.add_argument(
        "--job-limit",
        type=_positive_int,
    ...
    parser.add_argument(
        "--country",
        help="Country descriptor to collect. Defaults to the Philips source default.",
    )
    parser.add_argument(
        "--urls-path",
        type=Path,
