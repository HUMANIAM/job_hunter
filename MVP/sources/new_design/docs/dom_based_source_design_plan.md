...
Realization of `VacancyRetriever`.
...
`VacancyRetriever` should not know about facets, selectors, pages, or next
page URLs.
...
```text
BrowserAccess -> SiouxVacancyRetriever -> SiouxVacancySource
```
...
This cursor belongs to `SiouxVacancyRetriever`, not the shared base class.

## Retrieval Flow

### Initial Cursor

`SiouxVacancyRetriever._get_initial_cursor(criteria)` should:
...
`SiouxVacancyRetriever._retrieve_listing_batch(cursor, progress)` should:
...
Sioux-specific stop conditions belong in `SiouxVacancyRetriever`:
...
5. Add `SiouxVacancyRetriever` with private cursor state.
